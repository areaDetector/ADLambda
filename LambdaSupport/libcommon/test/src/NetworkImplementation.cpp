...
namespace DetCommonNS
{
    ...
    int NetworkTCPImplementation::Connect()
    {
        ...
        if(nRet < 0)
        {
            LOG_STREAM(__FUNCTION__,ERROR,"cannot connect to host!");
            return nRet;
        }
    ...
    int NetworkTCPImplementation::ReceiveData(char* ptrchData,int nLength)
    {
        ...
        while(nTotalReceivedPacket<nLength)
        {
            ...
            else
            {
                ...
                //cout<<"nTotalRecived:"<<nTotalReceivedPacket<<"This packet:"<<nRet<<endl;
            }
        }
        delete ptrChRec;
        return 0;
    ...
    int NetworkUDPImplementation::Connect()
    {
        ...
        if(nRet < 0)
        {    
            LOG_STREAM(__FUNCTION__,ERROR,"bind failed!");
            return nRet;
        }
