...
namespace FSDetCoreNS
{
    ...
    vector<int32> FileOperation::ReadDataFromIntBinaryFile()
    {
        LOG_TRACE(__FUNCTION__);
        throw std::runtime_error("not implemented");
    }

    vector<int16> FileOperation::ReadDataFromBinaryFile()
    {
        LOG_TRACE(__FUNCTION__);
        throw std::runtime_error("not implemented");
    }
    ...
    vector<int32> FileReader::ReadDataFromIntBinaryFile()
    {
        LOG_TRACE(__FUNCTION__);
        ...
        if(IsOpen()) {
            while(!m_ifFileReader.eof()) {
                m_ifFileReader.read(reinterpret_cast<char*>(&intTemp), sizeof(int32));
    ...
    vector<int16> FileReader::ReadDataFromBinaryFile()
    {
        LOG_TRACE(__FUNCTION__);
        ...
        if(IsOpen()) {
            while(!m_ifFileReader.eof()) {
                m_ifFileReader.read(reinterpret_cast<char*>(&shTemp), sizeof(int16));
                if(!m_ifFileReader.eof()) {
                    EndianSwap<int16>(&shTemp);
    ...
    void FileReader::EndianSwap(T* objT)
    {
        ...
        uchar* ptrchTemp = reinterpret_cast<uchar*>(objT);
    ...
    void FileWriter::WriteDataToFile()
    {
        ...
        for(vector<string>::iterator it = (this->m_vStrContent).begin();
            it != (this->m_vStrContent).end();
            it++)
            m_ofFileWriter << (*it) << "\n";
    }
}
