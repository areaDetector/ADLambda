...
namespace DetCommonNS
{
    ...
    ImageDecoder::ImageDecoder(vector<short> _vCurrentChips)
    {
        ...
        m_ptrchReorderedData = new unsigned char[m_nImgDataLength]; // In theory, new in C++ standard should automatically give at least 8-byte alignment.
        m_ptrshReshuffledData = new unsigned short[m_nImgLength];
        m_ptrshFullImg = new short[m_nImgLength];
    ...
    short* ImageDecoder::RunDecodingImg()
    {
        LOG_TRACE(__FUNCTION__);
        
        //refill temporary image data with 0
        memset(m_ptrchReorderedData,0,m_nImgDataLength);
        memset(m_ptrshReshuffledData,0,(m_nImgLength*2)); // We have short values - need to *2 to get no of bytes in image
    ...
    void ImageDecoder::ReorderRawData()
    {
        ...
        //Here, the raw data stream is reordered so that all the data from each chip is gathered together
        //first 32XNchips Bytes are headers for image
        //memcpy(m_ptrchReorderedData;m_ptrchRawImg;m_nChipHeaderLength);
        int nChipNo = 1;
        int nChipDataParts = 0;
        //don't need to process image header part
        for(int i=m_nChipHeaderLength;i<m_nTotalImgLength;i+=BLOCK_SIZE_IN_BYTES)
        {
            //calculate destination positions
            int nPos =(nChipNo-1)*BYTES_IN_CHIP //current chip No.
    ...
    void ImageDecoder::ReshuffleBitsInRawData()
    {
        ...
        int nBytesInRow = CHIP_SIZE * 12/8;
        ...
        unsigned char chTempByte;
            
        //go through all bytes
        for(int i=0;i<m_nImgDataLength;i++)
        {
    ...
    void ImageDecoder::BuildFullImage()
    {
        ...
        int nChipNo = 1;
        int nStartPos = m_vXstartPos[m_vCurrentUsedChips[nChipNo-1]-1]
            +  m_vYstartPos[m_vCurrentUsedChips[nChipNo-1]-1]*m_nModuleSizeX;
            
        int nRow = 0;
        int nOffset = 0;
        int nPos = nStartPos+nRow*m_nModuleSizeX;
        int nRotatedChipStartPos = 0;
            
            
        for(int i=0;i<m_nImgLength;i++)
        {    
            ...
            {    
                ...
                if(nRow == CHIP_SIZE)
                {
                    nRow = 0;
                    nChipNo++;
                    nStartPos = m_vXstartPos[m_vCurrentUsedChips[nChipNo-1]-1] +  m_vYstartPos[m_vCurrentUsedChips[nChipNo-1]-1]*m_nModuleSizeX;
    ...
    void ImageDecoder::BuildFullImageFast()
    {
        ...
        int nChipsPresent = m_vCurrentUsedChips.size();
	int i=0;
	int outPos=0;

	for(int nChip=0; nChip<nChipsPresent;nChip++)
	{
	    // Find index of start of chip in output array
	    int nStartPos = m_vXstartPos[m_vCurrentUsedChips[nChip]-1]
		...
		int irot=i;	        
	        ...
	        for(int nRow=0; nRow<CHIP_SIZE; nRow++)
	        {
		    // Start position of each row
		    outPos = nStartPos + nRow * m_nModuleSizeX;
		    for(int nCol=0; nCol<CHIP_SIZE; nCol++)
		    {
		        m_ptrshFullImg[outPos++] = m_ptrshReshuffledData[--irot]; // When working backwards using irot, pre-decrement to get right result
		    }
	        }
	    }
	    else
    ...
    void ImageDecoder::CalculateChipPos()
    {
        ...
        int nMinXStartPos = INT_MAX;
        int nMaxXStartPos = INT_MIN;
        int nMinYStartPos = INT_MAX;
        int nMaxYStartPos = INT_MIN;
        int nCurrentXStartPos = 0;
        int nCurrentYStartPos = 0;
        bool bCurrentChipRotated = 0;
            
        for(int i=0;i<m_nChipNumbers;i++)
        {
            ...
            nMinXStartPos = (nCurrentXStartPos < nMinXStartPos ? nCurrentXStartPos : nMinXStartPos);
            nMaxXStartPos = (nCurrentXStartPos > nMaxXStartPos ? nCurrentXStartPos : nMaxXStartPos);
            nMinYStartPos = (nCurrentYStartPos < nMinYStartPos ? nCurrentYStartPos : nMinYStartPos);
            nMaxYStartPos = (nCurrentYStartPos > nMaxYStartPos ? nCurrentYStartPos : nMaxYStartPos);
    ...
    vector<unsigned short> ImageDecoder::Generate12BitTable()
    {
        LOG_TRACE(__FUNCTION__);
        ...
        vector<unsigned short> vLFSR12bits(4096,0);
    ...
    // Here, we generate 64-bit LUT that allows us to take a nibble of data from the raw data stream and stick it into the short output stream directly
    // The key idea is that each bit in the raw data stream needs to be inserted into successive elements in a short array. 
    // This is currently done 1 bit at a time, but it should be possible to take 4 bits and look up a 64-bit word that can be inserted directly into the short array
    // For example, 0111 would give a lookup value of 00000000000000000 0000000000000001 0000000000000001 0000000000000001
    // Note that the routine constructs this LUT by creating short arrays and casting to long long. This ensures consistency regardless of endian format.

    vector<unsigned long long> ImageDecoder::GenerateReshuffleLUT()
    {
        LOG_TRACE(__FUNCTION__);
	...
	// Will use a short array to convert to 64-bit value
        ...
        uint LFSR=0;//init LFSR
	int tablesize = 16;
        vector<unsigned long long> vReshuffleLUT(tablesize,0);
	unsigned short shEntry[4];
	unsigned long long* ptrlEntry; // Used to convert short array into long value!
	ptrlEntry = reinterpret_cast<unsigned long long*>(&shEntry[0]);

	//std::cout << "Size of unsigned long long is: " << sizeof(unsigned long long) << "\n";

	unsigned long long summationvalue;
	    ...
	    //std::cout << "Lookup table entry " << i << " is long value " << vReshuffleLUT[i] << "\n";
        }
    ...
    // Our chip always has 256 pixel rows. This means 256 integer outputs in row, i.e. 512 bytes in output stream, i.e. 64 long words or 32 pairs.
    ...
    void ImageDecoder::ReshuffleBitsInRawDataFast()
    {
        ...
        //each pixel has 12 bits
        int nBytesInInputRow = CHIP_SIZE * 12/8;
	int nLongsInOutputRow = CHIP_SIZE*2/8; // A row of 256 short output values corresponds to 256*2/8 longs.

        //value from 0 - 255
        int nRow = 0;
        int nPos = 0; // This now keeps track of long pointer position
        
        unsigned char chTemp;
        unsigned char chTempByte;
	unsigned long long lLookupVal;
	unsigned long long* ptrlongReshuffledData; // long pointer used to process reshuffled data output
	ptrlongReshuffledData = reinterpret_cast<unsigned long long*>(m_ptrshReshuffledData);
